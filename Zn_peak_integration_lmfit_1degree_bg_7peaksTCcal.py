...
def single_peak_fit(x,y,hint,scan_i): 
    ...
    if scan_i>=0: 
        ...
        params['sigma'].min = 0.005
        params['sigma'].max = 0.015
        params['fraction'].min = 0
        params['fraction'].max = 1
        params['center'].min = hint['center']-0.01
        params['center'].max = hint['center']+0.01
    if scan_i>=1: 
        params['amplitude'].max = hint['amplitude']*2
    else: 
        pass
...
# set initial parameter values and bounds
    if scan_i >= 0:
        ...
        params['sigma1'].min = 0.005
        params['sigma1'].max = 0.015
        params['sigma2'].min = 0.005
        params['sigma2'].max = 0.015
        params['fraction1'].min = 0
        params['fraction1'].max = 1
        params['fraction2'].min = 0
        params['fraction2'].max = 1
        params['center1'].min = hint['center1']-0.005
        params['center1'].max = hint['center1']+0.005
        params['center2'].min = hint['center2']-0.005
        params['center2'].max = hint['center2']+0.005
...
def peak_intensities_lmfit(dataframe, intervals,hint):
    ...
    for (start, stop) in intervals: 
        ...
        for i in reversed(list(range(yy.shape[1]))): 
            
            print('fitting interval: ', str([start,stop]))
            print('currently working on', i+1, 'th patterns of total', yy.shape[1], 'patterns')
            pattern=patterns[i]
...
def extract_time_abs(dataframe,start_depo_real): 
    
    header = dataframe.columns.values.tolist()
    header.pop(0)
    time_list=[re.sub(r'(\S+)(\d{8})-(\d{6})(\S+)',r'\2'+'-' + r'\3', i) for i in header]
    

    time_depo=pd.DataFrame()
    time_depo['Time']=(pd.to_datetime(time_list, format='%Y%m%d-%H%M%S'))

    time_depo['Time_real']=time_depo['Time']-start_depo_real

    time_depo['Time/min'] = time_depo['Time_real'].dt.total_seconds().div(60)
